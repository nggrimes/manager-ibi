## sim_v3 Simulations for the "fish" quota choices with fishers maximizing insurance coverage
...
for(j in 1:ncol(rnd_weather)){
  ...
  f_pub<-spline(x=b,y=noi_pol$fstar,xout=sim$pub_b[1])$y
  f_pub_ins<-spline(x=b,y=noi_pol$fstar,xout=sim$pub_ins_b[1])$y
  
  cov_ins<-spline(x=b,y=ins_pol$cstar,xout = sim$ins_b[1])$y
  cov_pub_ins<-spline(x=b,y=noi_pol$cstar,xout = sim$pub_ins_b[1])$y
  ...
  ins_pay<-map_dbl(.x=shock,
                  .f=~insurance_payout(shock=.x,
                                       coverage=cov_ins,
                                       trigger=trigger))
  ins_prem<-cov_ins*prem_rate
  
  pub_pay<-map_dbl(.x=shock,
                   .f=~insurance_payout(shock=.x,
                                        coverage=cov_pub_ins,
                                        trigger=trigger))
  pub_prem<-cov_ins*prem_rate
  ...
  pi_pub_ins<-map2_dbl(.x=shock,.y=pub_pay,~f_pub_ins-c_cost*f_pub_ins/((1+.x)*sim$pub_b[1])+.y-ins_prem)
  
  sim$pub_ins_ut[1]<-sum(map_dbl(pi_pub_ins,~utility(.x,gamma,boost=0,mod=parameters$ut_mod))*w_probs)
  
  
  pi_pub<-map2_dbl(.x=shock,.y=0,~f_pub-c_cost*f_pub/((1+.x)*sim$pub_b[1])+.y-0)
  ...
  for(i in 2:n_years){
    ...
    sim$pub_b[i] <- growth(max(sim$pub_b[i-1]*(1+rnd_weather[i-1,j])-f_pub,parameters$K/1000),parameters$r,parameters$K)
    sim$pub_ins_b[i] <- growth(max(sim$pub_ins_b[i-1]*(1+rnd_weather[i-1,j])-f_pub_ins,parameters$K/1000),parameters$r,parameters$K)

    f_ins<-spline(x=b,y=ins_pol$fstar,xout=sim$ins_b[i])$y
    f_pub<-spline(x=b,y=noi_pol$fstar,xout=sim$pub_b[i])$y
    f_pub_ins<-spline(x=b,y=noi_pol$fstar,xout=sim$pub_ins_b[i])$y
    
    cov_ins<-spline(x=b,y=ins_pol$cstar,xout = sim$ins_b[i])$y
    cov_pub_ins<-spline(x=b,y=noi_pol$cstar,xout = sim$pub_ins_b[i])$y
    ...
    pi_pub_ins<-map2_dbl(.x=shock,.y=pub_pay,~f_pub_ins-c_cost*f_pub_ins/((1+.x)*sim$pub_b[i])+.y-pub_prem)
    
    sim$pub_ins_ut[i]<-sum(map_dbl(pi_pub_ins,~utility(.x,gamma,boost=0,mod=parameters$ut_mod))*w_probs)
    
    
    pi_pub<-map2_dbl(.x=shock,.y=0,~f_pub-c_cost*f_pub/((1+.x)*sim$pub_b[i])+.y-0)
  ...
  final_b_pub_ins[j]<-sim$pub_b[n_years]
  
  final_ut_pub_no[j]<-sum(sim$pub_ut_disc)
  final_b_pub_no[j]<-sim$pub_ins_b[n_years]

  
}

df<-data.frame(final_ut, final_b, final_ut_pub_ins, final_b_pub_no,final_ut_pub_no,final_b_pub_ins,sim=1:n_sims,b0=b0) |> 
  drop_na() |> 
  pivot_longer(cols=c(final_ut, final_b, final_ut_pub_ins, final_b_pub_no,final_ut_pub_no,final_b_pub_ins), names_to='variable', values_to='value')
...
ggsave(here::here('fig','pre_shock_ut_sim_ra03_m5.png'),dpi=300,width=7,height=7)
...
ggsave(here::here('fig','pre_shock_b_sim_ra03_m5.png'),dpi=300,width=7,height=7)
