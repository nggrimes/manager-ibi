...
Payoff = function(f,b,theta,V){
  ...
  utility=-(1-exp(-a*(temp_prof))+Vfuture)
...
ggplot(temp_ins_t,aes(x=b,y=fstar,color=theta,group=theta))+geom_path()+scale_color_gradient2(midpoint=0,low='red',high='blue',mid = 'grey')


no_ins$ins='no'
h<-rbind(temp_ins_t,no_ins)

ggplot(h,aes(x=b,y=fstar,color=theta,group=theta))+geom_path()+scale_color_gradient2(midpoint=0,low='red',high='blue',mid = 'grey')+facet_wrap(~ins)
