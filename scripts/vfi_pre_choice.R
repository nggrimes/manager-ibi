...
vfi_pre_choice<-function(sizex,n_z,mean_theta,sigma_theta,p,c,cshape,r,K,ra,delta,m,trigger){
#### Parameters ####
set.seed(123)
...
solve_vfi <- function(with_insurance = TRUE) {
  ...
  V <- matrix(0, n_biomass, 1)
  V_new <- matrix(0, n_biomass, 1)
  
  policy_effort <- matrix(0, n_biomass, 1)
  policy_harvest <- matrix(0, n_biomass, 1)
  policy_profit <- matrix(0, n_biomass, 1)
  
  if (with_insurance) {
    policy_coverage <- matrix(0, n_biomass, 1)
    policy_premium <- matrix(0, n_biomass, 1)
  }
  
  
  # Objective function to MINIMIZE (negative of value function)
  objective_fn <- function(choices, current_biomass, V_current) {
    ...
    # Expected value over shock realizations
    EV <- 0
    expected_ut <- 0
    ...
    for (i_w in 1:n_shocks) {
      ...
      V_next <- spline(biomass_grid, V_current[, 1], xout = biomass_next,
                       method = "natural")$y
      
      # Accumulate
      EV <- EV + prob * V_next
      expected_ut<- prob*utility(profit_total,gamma)+expected_ut
    }
    
    
    # Total value
    total_value <- expected_ut + beta * EV
  ...
  for (iter in 1:max_iter) {
    ...
    for (i_b in 1:n_biomass) {
      ...
      if (iter == 1) {
        ...
        if (with_insurance) {
          init_guess <- c(policy_effort[i_b, 1], policy_coverage[i_b, 1])
        } else {
          init_guess <- c(policy_effort[i_b, 1])
        }
      }
      ...
      # Store results
      V_new[i_b, 1] <- max_value
      policy_effort[i_b, 1] <- best_effort
      
      if (with_insurance) {
        policy_coverage[i_b, 1] <- best_coverage
        policy_premium[i_b, 1] <- premium
      }
    }
    
    # Check convergence
    diff <- mean(abs(V_new - V))
