...
theta_raw<-rlnorm(n_sims,mean=mean_theta,sd=sigma_theta)
theta_dist<-theta_raw[which(theta_raw<2.5)]
...
growth = function(b,f,theta)
{
  ...
  escapement=b*theta-harvest(f=f,biomass=b,theta = theta)
...
Payoff = function(f,b,theta,V)
{
  ...
  if(temp_prof<=1){
    ...
    utility=-(log(temp_prof+250)+sum(Vfuture))
  }
...
while(error=="False"){
  ...
  for(j in 1:length(theta_prop)){
    ...
    for(i in 1:length(xgrid)){
      ...
      low=.0001
      high= 1
      output = optim(par=guess,fn=Payoff,lower=low,upper=high,b=b,V=V,theta=temptheta,method='L-BFGS-B')
      fstar = output$par
      Vstar = -output$value
      Vnext[i,j]=Vstar
