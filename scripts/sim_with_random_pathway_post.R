...
for(j in 1:ncol(rnd_weather)){
  ...
  pi_ins<-f_ins-parameters$c_cost*f_ins/((1+rnd_weather[1,j])*sim$ins_b[1])
  pi_pub<-f_noi-parameters$c_cost*f_noi/((1+rnd_weather[1,j])*sim$pub_b[1])

  ins_pay<-insurance_payout(rnd_weather[1,j],
                            coverage=c_ins,
                            trigger=parameters$trigger)
  
  pub_pay<-insurance_payout(rnd_weather[1,j],
                            coverage=c_noi,
                            trigger=parameters$trigger)
  ...
  sim$ins_ut[1]<-utility(pi_ins+ins_pay-ins_prem,
                         gamma=parameters$gamma,
                         boost=0,
                         mod=parameters$ut_mod)
  sim$pub_ut[1]<-utility(pi_pub,
                         gamma=parameters$gamma,
                         boost=0,
                         mod=parameters$ut_mod)
  sim$pub_ins_ut[1]<-utility(pi_pub+pub_pay-pub_prem,
                             gamma=parameters$gamma,
                             boost=0,
                             mod=parameters$ut_mod)
  ...
  for(i in 2:n_years){
    ...
    pi_ins<-f_ins-parameters$c_cost*f_ins/((1+rnd_weather[i,j])*sim$ins_b[i])
    pi_pub<-f_noi-parameters$c_cost*f_noi/((1+rnd_weather[i,j])*sim$pub_b[i])
  
    
    ins_pay<-insurance_payout(rnd_weather[i,j],
                              coverage=c_ins,
                              trigger=parameters$trigger)
    
    pub_pay<-insurance_payout(rnd_weather[i,j],
                              coverage=c_noi,
                              trigger=parameters$trigger)
    ins_prem<-c_ins*prem_rate
    ...
    sim$ins_ut[i]<-utility(pi_ins+ins_pay-ins_prem,
                           gamma=parameters$gamma,
                           boost=0,
                           mod=parameters$ut_mod)
    sim$pub_ut[i]<-utility(pi_pub,
                           gamma=parameters$gamma,
                           boost=0,
                           mod=parameters$ut_mod)
    
    sim$pub_ins_ut[i]<-utility(pi_pub+pub_pay-pub_prem,
                               gamma=parameters$gamma,
                               boost=0,
                               mod=parameters$ut_mod)
...
df |> filter(variable %in% c('final_ut', 'final_ut_pub_no','final_ut_pub_ins')) |> 
  ...
  scale_fill_manual(name="",values=c("#003660","#79A540",'red'),labels=c("final_ut"="Insurance","final_ut_pub_no"="No Insurance","final_ut_pub_ins"="Published Q, Insurance"))+
  theme_classic()+
